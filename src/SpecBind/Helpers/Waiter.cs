...
namespace SpecBind.Helpers
{
    ...
    public class Waiter
    {
        ...
        public void WaitFor(Func<bool> conditionChecker)
        {
            ...
            // We must wait for conditions in the same thread to support Microsoft Coded UI tests
        ...
        /// <summary>
        /// Waits for the given action to return successfully, or until the timeout is reached.
        /// </summary>
        /// <param name="action">The action to check.</param>
        /// <exception cref="TimeoutException">The condition was not met before the timeout was reached.</exception>
        public void WaitFor(Func<ActionResult> action)
        {
            this.WaitFor(() => action().Success);
        }
    }
    ...
    public class Waiter<T>
    {
        ...
        public void WaitFor(T element, Func<T, bool> conditionChecker)
        {
            ...
            throw new TimeoutException("Timed out after " + this.Timeout, lastException);
        }
    }
}